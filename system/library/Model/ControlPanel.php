 ...
 */
class Model_ControlPanel extends Model_Abstract
{
     ...
     */
    public function getControlPanelLogins($website_id)
    {
        $sth = self::getConnection()->prepare("SELECT * FROM `control_panels` WHERE `website_id` = :id");
     ...
     * @return array
     */
    public function getControlPanelDetails($id, $website_id = null)
    {
        if ($website_id) {
            $sth = self::getConnection()->prepare("SELECT * FROM `control_panels` WHERE `id` = :id AND `website_id` = :website_id");
            $sth->bindValue(":website_id", $website_id, PDO::PARAM_INT);
        } else {
            $sth = self::getConnection()->prepare("SELECT * FROM `control_panels` WHERE `id` = :id");
        }
    ...
    public function addControlPanelLogin($website_id, array $data)
    {
        ...
        $sth = self::getConnection()->prepare("INSERT INTO `control_panels` (`username`,`password`,`url`,`notes`, `website_id`) VALUES(:username,:password,:url,:notes,:website_id)");
        $sth->execute($data);
        $id = self::getConnection()->lastInsertId();
        return $this->getControlPanelDetails($id, $website_id);
    }
    ...
    public function updateControlPanelLogin($id, array $data, $website_id)
    {
        ...
        $data['id'] = $id;
        $sth = self::getConnection()->prepare("UPDATE `control_panels` SET `username` = :username,`password` = :password, `url` = :url, `notes` = :notes WHERE `id` = :id");
        $sth->execute($data);
        return $this->getControlPanelDetails($id);
    }
     ...
     * @return void
     */
    public function deleteControlPanelLogin($id)
    {
        // Delete admin logins
        $sth = self::getConnection()->prepare("DELETE FROM `control_panels` WHERE `id` = :id");
        $sth->bindValue(":id", $id, PDO::PARAM_INT);
        $sth->execute();
    }
